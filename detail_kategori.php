            ...
            <div class="middle-panel">
                <div class="bottom-middle-panel">
                    <ul>
						...
						$qberita="select *,(select kategori from tbl_kategori where id=tbl_berita.berita_kategori limit 1) kat,(select nama from tbl_user where username=tbl_berita.berita_username) as nama from tbl_berita where md5(berita_kategori)='$id' order by berita_id desc limit 100";
						$rberita=mysqli_query($konek,$qberita);
						while ($berita=mysqli_fetch_row($rberita)){
						$id=$berita[0];	
						?>
                         <li>
                            <div class="date"><?php echo konversi_tanggal("D, d-m-Y",$berita[6]).', By  '.$berita[15];?></div>
							<div class="title"><a href="detail_artikel.php?action=detail&id=<?php echo md5($id);?>"><?php echo html_entity_decode($berita[1]);?></a></div>
                            <div class="text">
							<img src="<?php echo 'include/berita.php?id='.$id ;?>" width="150px" height="90px">
							<?php echo html_entity_decode($berita[3]).'...';?></div>
							 <div class="link">
